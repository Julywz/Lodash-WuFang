var WuFang = {
    ...
    /**
     * 数组从右端开始，数组中的每一项都运行函数f，若函数返回假，则保留
     * @param  迭代的数组
     * @param  运行的函数
     * @return 返回运行为假的元素的数组
     * var users = [
    ...
    dropRightWhile: function(arr, f) {
      ...
      return arr.reverse().filter(a => !fn(a))
    },
    /**
     * 数组从头开始，数组中的每一项都运行函数f，若函数返回假，则保留
     * @param  迭代的数组
     * @param  运行的函数
     * @return 返回运行为假的元素的数组
     * var users = [
    ...
    dropWhile: function(arr, f) {
      ...
      return arr.filter(a => !fn(a))
    },
    // find: function(arr, f, index) {
    //   return arr.filter().indexOf()
    // },
    /**
     * 将数组中的某几个元素替换成别的值
    ...
    /**
     * 用链表的形式表示数组，用最笨的方法做的，先循环一次将每个数组的元素变成一个对象{value:value,next:null}
     * 再循环一次将next指向下一个元素
     * @param  数组
     * @return 返回数组的链表形式
     */
    ...
    /**
     * 用链表的形式表示数组，用数组的map和reduce方法做的，先将每个数组的元素映射成一个对象{value:value,next:null}
     * 再用reduce将next指向下一个元素
     * @param  数组
     * @return 返回数组的链表形式
     */
    arrayToLinkedList2: function(arr) {
      if (arr.length == 0) {
        return {
          next: null
        }
...
console.log(WuFang.parseJson(json))
